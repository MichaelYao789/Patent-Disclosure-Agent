...
class LLMClient:
    ...
    def _normalize_json_text(self, text: str) -> str:
        """
        从模型返回文本中提取纯 JSON 字符串
        兼容模型常见的 ```json 代码块包裹或前后夹杂说明文字的情况。
        """
        if not text:
            return ""
        text = text.strip()
    ...
    def get_json_response(self, messages: List[Dict], 
                          expected_keys: Optional[List[str]] = None) -> Any:
        """
        强制获取 JSON 格式响应（兼容数组或对象）。
        
        参数：
            messages: 消息列表
            expected_keys: 期望存在的键列表，用于校验（仅对象返回时生效）
        ...
        for attempt in range(3):
            try:
                ...
                raw = self._normalize_json_text(content)
                if not raw:
