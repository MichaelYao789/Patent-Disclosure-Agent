...
class RetrievalTools:
    ...
    def patent_us_search(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        # 使用 JSON 格式查询参数，并对中文/特殊字符做正确编码
        query_obj = {"_text_any": {"patent_title": query}}
        fields = ["patent_number", "patent_title", "patent_abstract",
                  "patent_date", "inventors"]
        options = {"per_page": limit}
        params = {
            "q": json.dumps(query_obj, ensure_ascii=False),
            "f": json.dumps(fields, ensure_ascii=False),
            "o": json.dumps(options, ensure_ascii=False),
        }
        url = f"{self.patentsview_base}?{urlencode(params)}"
        try:
